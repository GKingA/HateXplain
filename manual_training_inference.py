...
def get_gpu():
    ...
    while 1:
        ...
        if len(tempID) > 0:
            print("Found a gpu")
            print(
                "We will use the GPU:", tempID[0], torch.cuda.get_device_name(tempID[0])
            )
            deviceID = tempID
            return deviceID
        else:
...
def train_model(params, device):
    ...
    for epoch_i in range(0, params["epochs"]):
        ...
        (
            ...
            val_roc_auc,
            _,
        ) = Eval_phase(params, "val", model, validation_dataloader, device)
        (
            test_fscore,
            test_accuracy,
            test_precision,
...
if __name__ == "__main__":
    my_parser = argparse.ArgumentParser(
        description="Train a deep-learning model with the given data"
    )
