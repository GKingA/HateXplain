...
def distribute(old_distribution, new_distribution, index, left, right, params):
    ...
    if method == "geometric":
        ...
        for temp in range(index - left, index):
            new_distribution[temp] = (
                new_distribution[temp] + reserve * newprob[-(temp - (index - left)) - 1]
            )
        ...
        for temp in range(index + 1, index + right):
            new_distribution[temp] = (
                new_distribution[temp] + reserve * newprob[temp - (index + 1)]
            )
...
def decay(old_distribution, params):
    ...
    for index in range(len(old_distribution)):
        ...
        new_distribution = distribute(
            old_distribution, new_distribution, index, left, right, params
        )
