...
def get_annotated_data(params):
    ...
    for key in data:
        ...
        final_label = []
        for i in range(1, 4):
            temp["annotatorid" + str(i)] = data[key]["annotators"][i - 1][
                "annotator_id"
            ]
...
def convert_data(test_data, params, list_dict, rational_present=True, topk=2, bert_mask=False):
    ...
    for ele in list_dict:
        temp_dict[ele["annotation_id"]] = ele["rationales"][0][
    ...
    for index, row in tqdm(data.iterrows(), total=len(data)):
        ...
        for i in range(1, 4):
            ...
            if row["final_annotation"] not in ["normal", "non-toxic", "undecided"]:
                ...
                elif row["old_vs_new"] == "old" and (
                    row["pred" + str(i)] not in ["normal", "non-toxic"]
                ):
                    rational_list.append(attention_masks[i - 1])
