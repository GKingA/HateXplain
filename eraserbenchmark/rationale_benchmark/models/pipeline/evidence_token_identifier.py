...
def _get_sampling_method(
    ...
    if training_pars["sampling_method"] == "everything":
        ...
        def everything_sampler(
            document: List[SentenceEvidence], _: Dict[str, List[SentenceEvidence]]
        ...
        raise ValueError(
            f"Unknown sampling method for training: {training_pars['sampling_method']}"
        )


def train_evidence_token_identifier(
    ...
    def _prep_data_for_epoch(
        evidence_data: Dict[str, Dict[str, List[SentenceEvidence]]],
        sampler: Callable[
            [List[SentenceEvidence], Dict[str, List[SentenceEvidence]]],
            List[SentenceEvidence],
        ],
    ) -> List[SentenceEvidence]:
        output_sentences = []
    ...
    logging.info(
        f"Beginning training with {len(train)} annotations, {len(val)} for validation"
    )
    ...
    model_save_file = os.path.join(
        evidence_identifier_output_dir, "evidence_token_identifier.pt"
    )
    epoch_save_file = os.path.join(
        evidence_identifier_output_dir, "evidence_token_identifier_epoch_data.pt"
    )

    if optimizer is None:
        optimizer = torch.optim.Adam(
            evidence_token_identifier.parameters(),
            lr=model_pars["evidence_token_identifier"]["lr"],
        )
    ...
    use_cose_hack = bool(
        model_pars["evidence_token_identifier"].get("cose_data_hack", 0)
    )

    # annotation id -> docid -> [SentenceEvidence])
    evidence_train_data = annotations_to_evidence_token_identification(
        train,
        source_documents=source_documents,
        interned_documents=interned_documents,
        token_mapping=token_mapping,
    )
    evidence_val_data = annotations_to_evidence_token_identification(
        val,
        source_documents=source_documents,
        interned_documents=interned_documents,
        token_mapping=token_mapping,
    ...
    if os.path.exists(epoch_save_file):
        ...
        best_model_state_dict = OrderedDict(
            {k: v.cpu() for k, v in evidence_token_identifier.state_dict().items()}
        )
    logging.info(
        f"Training evidence identifier from epoch {start_epoch} until epoch {epochs}"
    )
    optimizer.zero_grad()
    for epoch in range(start_epoch, epochs):
        ...
        for batch_start in range(0, len(epoch_train_data), batch_size):
            ...
            if max_grad_norm:
                torch.nn.utils.clip_grad_norm_(
                    evidence_token_identifier.parameters(), max_grad_norm
                )
            optimizer.step()
            if scheduler:
                scheduler.step()
        ...
        with torch.no_grad():
            ...
            results["epoch_val_acc"].append(
                accuracy_score(epoch_val_truth, epoch_val_hard_pred)
            )
            results["epoch_val_f"].append(
                classification_report(
                    epoch_val_truth, epoch_val_hard_pred, output_dict=True
                )
            )
            epoch_val_soft_pred_for_scoring = [
                [[1 - z, z] for z in y] for y in epoch_val_soft_pred
            ]
            logging.info(
            ...
            if epoch_val_loss < best_val_loss:
                logging.debug(
                    f"Epoch {epoch} new best model with val loss {epoch_val_loss}"
                )
                best_model_state_dict = OrderedDict(
                    {
                        k: v.cpu()
                        for k, v in evidence_token_identifier.state_dict().items()
                    }
                )
                best_epoch = epoch
