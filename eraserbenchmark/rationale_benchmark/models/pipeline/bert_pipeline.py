...
from rationale_benchmark.models.pipeline.evidence_token_identifier import (
    train_evidence_token_identifier,
)
...
from rationale_benchmark.models.pipeline.pipeline_utils import (
    decode,
    decode_evidence_tokens_and_classify,
)
...
def initialize_models(params: dict, batch_first: bool, unk_token="<unk>"):
    ...
    if bool(params["use_evidence_sentence_identifier"]):
        use_half_precision = bool(
            params["evidence_identifier"].get("use_half_precision", 1)
        )
    ...
    if bool(params["use_evidence_token_identifier"]):
        use_half_precision = bool(
            params["evidence_token_identifier"].get("use_half_precision", 1)
        )
        evidence_token_identifier = BertTagger(
            bert_dir=bert_dir,
            pad_token_id=pad_token_id,
            cls_token_id=cls_token_id,
            sep_token_id=sep_token_id,
    ...
    use_half_precision = bool(
        params["evidence_classifier"].get("use_half_precision", 1)
    )
...
def bert_intern_doc(
    ...
    return [
        list(
            chain.from_iterable(
                special_token_map.get(w, tokenizer.encode(w)) for w in s
            )
        )
        for s in doc
    ]
...
def bert_intern_annotation(annotations: List[Annotation], tokenizer):
    ...
    for ann in annotations:
        ...
        for ev_group in ann.evidences:
            ...
            for ev in ev_group:
                text = list(
                    chain.from_iterable(tokenizer.tokenize(w) for w in ev.text.split())
                )
                if len(text) == 0:
                    continue
        ...
        query = list(
            chain.from_iterable(tokenizer.tokenize(w) for w in ann.query.split())
        )
        if len(query) > 0:
...
def main():
    ...
    * decode first the evidence, then run classification for each split
    ...
    (
        ...
        evidence_classes,
        tokenizer,
    ) = initialize_models(model_params, batch_first=BATCH_FIRST, unk_token=unk_token)
    if not ((evidence_identifier is None) ^ (evidence_token_identifier is None)):
        raise ValueError(
            "Exactly one of the evidence identifier and evidence token identifier must be defined, not both!"
        )
    ...
    if os.path.exists(cache):
        ...
        for d, doc in documents.items():
            tokenized, w_slices = bert_tokenize_doc(
                doc, tokenizer, special_token_map=special_token_map
            )
            interned_documents[d] = bert_intern_doc(
                tokenized, tokenizer, special_token_map=special_token_map
            )
    ...
    if evidence_token_identifier is not None:
        ...
        (
            evidence_token_identifier,
            evidence_token_identifier_results,
        ) = train_evidence_token_identifier(
            evidence_token_identifier,
            args.output_dir,
            interned_train,
    ...
    if evidence_identifier is not None:
        ...
        use_cose_hack = bool(
            model_params["evidence_token_identifier"].get("cose_data_hack", 0)
        )
        if use_cose_hack:
            logger.info("Using COS-E data prep and processing hacks!")
            logger.info(
                "These hacks impact evidence identification, classification, and decoding. They are not general"
            )
        (
            pipeline_results,
            train_decoded,
            ...
            evidence_token_identifier.cuda(),
            evidence_classifier.cuda(),
            interned_train,
            interned_val,
            interned_test,
