...
def train_evidence_classifier(
    ...
    token_only_evidence: bool = False,
    ...
    """
    logging.info(
        f"Beginning training evidence classifier with {len(train)} annotations, {len(val)} for validation"
    )
    ...
    model_save_file = os.path.join(
        evidence_classifier_output_dir, "evidence_classifier.pt"
    )
    epoch_save_file = os.path.join(
        evidence_classifier_output_dir, "evidence_classifier_epoch_data.pt"
    )

    device = next(evidence_classifier.parameters()).device
    if optimizer is None:
        optimizer = torch.optim.Adam(
            evidence_classifier.parameters(), lr=model_pars["evidence_classifier"]["lr"]
        )
    ...
    if token_only_evidence:
        evidence_train_data = token_annotations_to_evidence_classification(
            train, documents, class_interner
        )
        evidence_val_data = token_annotations_to_evidence_classification(
            val, documents, class_interner
        )
    else:
        evidence_train_data = annotations_to_evidence_classification(
            train, documents, class_interner, include_all=False
        )
        evidence_val_data = annotations_to_evidence_classification(
            val, documents, class_interner, include_all=False
        )
    ...
    for epoch in range(start_epoch, epochs):
        epoch_train_data = random.sample(
            evidence_train_data, k=len(evidence_train_data)
        )
        ...
        with torch.no_grad():
            ...
            results["train_acc"].append(
                accuracy_score(epoch_train_truth, epoch_train_hard_pred)
            )
            (
                epoch_val_loss,
                epoch_val_soft_pred,
