...
def _get_sampling_method(
    ...
    if training_pars["sampling_method"] == "random":
        ...
        logging.info(
            f"Setting up random sampling with negative/positive ratio = {sampling_ratio}"
        )
...
def train_evidence_identifier(
    ...
    model_save_file = os.path.join(
        evidence_identifier_output_dir, "evidence_identifier.pt"
    )
    epoch_save_file = os.path.join(
        evidence_identifier_output_dir, "evidence_identifier_epoch_data.pt"
    )

    if optimizer is None:
        optimizer = torch.optim.Adam(
            evidence_identifier.parameters(), lr=model_pars["evidence_identifier"]["lr"]
        )
    ...
    if os.path.exists(epoch_save_file):
        ...
        best_model_state_dict = OrderedDict(
            {k: v.cpu() for k, v in evidence_identifier.state_dict().items()}
        )
    logging.info(
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for batch_start in range(0, len(epoch_train_data), batch_size):
            ...
            if max_grad_norm:
                torch.nn.utils.clip_grad_norm_(
                    evidence_identifier.parameters(), max_grad_norm
                )
            optimizer.step()
            if scheduler:
                scheduler.step()
        ...
        with torch.no_grad():
            ...
            logging.info(
                f"Epoch {epoch} sampled val loss {sampled_epoch_val_loss}, acc {sampled_epoch_val_acc}"
            )
            # evaluate over *all* of the validation data
            all_val_data = list(
                filter(
                    lambda se: len(se.sentence) > 0,
                    chain.from_iterable(
                        chain.from_iterable(
                            x.values() for x in evidence_val_data.values()
                        )
                    ),
                )
            )
            ...
            if sampled_epoch_val_loss < best_val_loss:
                logging.debug(
                    f"Epoch {epoch} new best model with sampled val loss {sampled_epoch_val_loss}"
                )
                best_model_state_dict = OrderedDict(
                    {k: v.cpu() for k, v in evidence_identifier.state_dict().items()}
                )
