...
class WordEmbedder(nn.Module):
    ...
    def forward(self, docids: List[Any], ps: PaddedSequence) -> PaddedSequence:
        ...
        if docids and len(docids) not in ps.data.size():
            raise ValueError(
                f"Document id dimension {len(docids)} does not match input data dimensions {ps.data.size()}"
            )
...
class LuongAttention(nn.Module):
    def __init__(
        self, output_size: int, query_size: int = None, dropout_rate: float = 0.0
    ):
    ...
    def forward(
        self, output: PaddedSequence, query: torch.Tensor = None  # batch x length x rep
    ):
...
class RNNEncoder(nn.Module):
    ...
    def forward(self, docids: List[Any], docs: PaddedSequence, query: torch.Tensor):
        ...
        output = PaddedSequence.from_packed_sequence(
            output, batch_first=docs.batch_first
        )
        assert hidden.size()[-1] == self.rnn.hidden_size
        if self.attention_mechanism is not None:
            unnormalized_attention, attention, hidden = self.attention_mechanism(
                output, query
            )
            assert hidden.size()[-1] == self.rnn.hidden_size
        else:
...
class AttentiveClassifier(nn.Module):
    ...
    def __init__(
        ...
        document_output_dimension = (
            self.document_encoder.output_dimension if document_encoder else 0
        )
        query_output_dimension = (
            self.query_encoder.output_dimension if query_encoder else 0
        )
    ...
    def forward(
        ...
        if query is not None:
            ...
            query = PaddedSequence.autopad(
                query, batch_first=self.query_encoder.batch_first, device=device
            )
            (
                query_vector,
                unnormalized_query_attention,
                normalized_query_attention,
                _,
            ) = self.query_encoder(None, query, None)
            unnormalized_query_attention = PaddedSequence(
                unnormalized_query_attention, query.batch_sizes, query.batch_first
            )
            normalized_query_attention = PaddedSequence(
                normalized_query_attention, query.batch_sizes, query.batch_first
            )
        else:
            query_vector = None
        ...
        if document_batch is not None:
            ...
            document_batch = PaddedSequence.autopad(
                document_batch,
                batch_first=self.document_encoder.batch_first,
                device=device,
            )
            (
                document_vector,
                ...
                _,
            ) = self.document_encoder(docids, document_batch, query_vector)
            unnormalized_document_attention = PaddedSequence(
                unnormalized_document_attention,
                document_batch.batch_sizes,
                document_batch.batch_first,
            )
            normalized_document_attention = PaddedSequence(
                normalized_document_attention,
                document_batch.batch_sizes,
                document_batch.batch_first,
            )
        ...
        if query_vector is not None and document_vector is not None:
            ...
            combined_vector = (
                query_vector if query_vector is not None else document_vector
            )
