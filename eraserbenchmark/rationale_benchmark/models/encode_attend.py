...
def initialize_model(params: dict, vocab: Set[str], batch_first: bool, unk_token="UNK"):
    ...
    encoding_size = params["classifier"].get(
        "hidden_size", word_embedder.output_dimension
    )
    ...
    evidence_classes = dict(
        (y, x) for (x, y) in enumerate(params["classifier"]["classes"])
    )
...
def annotation_to_instances(
    ann: Annotation, docs: Dict[str, List[List[int]]], class_interner: Dict[str, int]
):
...
def convert_for_training(
    ...
    for ann in annotations:
        kls, flattened_docs, ev_spans = annotation_to_instances(
            ann, docs, class_interner
        )
...
def train_classifier(
    ...
    logging.info(
        f"Beginning training classifier with {len(train)} annotations, {len(val)} for validation"
    )
    ...
    (
        ...
        train_queries,
        train_docs,
        train_evidence_spans,
    ) = convert_for_training(train, documents, class_interner)
    (
        val_ids,
        val_classes,
        val_queries,
        val_docs,
        val_evidence_spans,
    ) = convert_for_training(val, documents, class_interner)
    ...
    if attention_optimizer is None:
        attention_optimizer = torch.optim.Adam(
            classifier.parameters(), lr=model_pars["classifier"]["lr"]
        )
    ...
    if os.path.exists(epoch_save_file):
        ...
        logging.info(
            f"Restoring training from attention epoch {start_attention_epoch} / {start_classifier_epoch}"
        )
    logging.info(
        f"Training classifier attention from epoch {start_attention_epoch} until epoch {attention_epochs}"
    )
    for attention_epoch in range(start_attention_epoch, attention_epochs):
        ...
        for batch_start in range(0, len(train_ids), batch_size):
            ...
            _, _, _, unnormalized_document_attention, _ = classifier(
                queries, None, docs, return_attentions=True
            )
            partially_normalized_document_attention = torch.sigmoid(
                unnormalized_document_attention.data.squeeze()
            )
            train_spans = PaddedSequence.autopad(
                train_spans,
                batch_first=True,
                device=unnormalized_document_attention.data.device,
            )
            batch_loss = attention_criterion(
                partially_normalized_document_attention, train_spans.data.float()
            )
        ...
        logging.info(
            f"Epoch {attention_epoch} attention train loss {epoch_train_loss / epoch_train_tokens}"
        )
        with torch.no_grad():
            classifier.eval()
            for batch_start in range(0, len(val_ids), batch_size):
                ...
                unnormalized_document_attention = torch.sigmoid(
                    unnormalized_document_attention.data
                )
                val_spans = PaddedSequence.autopad(
                    val_spans, batch_first=True, device=device
                )
                batch_loss = attention_criterion(
                    unnormalized_document_attention.squeeze(), val_spans.data.float()
                )
            ...
            if epoch_val_loss < best_attention_loss:
                ...
                logging.info(
                    f"Epoch {attention_epoch} new best model with val loss {epoch_val_loss}"
                )
        if attention_epoch - best_attention_epoch > patience:
            ...
            break
    logging.info(
        f"Training classifier from epoch {start_classifier_epoch} until epoch {epochs}"
    )
    for classifier_epoch in range(start_classifier_epoch, epochs):
        ...
        with torch.no_grad():
            ...
            logging.info(
                f"Epoch {classifier_epoch} val loss {epoch_val_loss}, accuracy: {val_accuracy}, f1: {val_f1}"
            )
            if epoch_val_loss < best_classifier_loss:
                ...
                torch.save(epoch_data, epoch_save_file)
                logging.info(
                    f"Epoch {classifier_epoch} new best model with val loss {epoch_val_loss}"
                )
        if classifier_epoch - best_classifier_epoch > patience:
            logging.info(
                f"Exiting after epoch {classifier_epoch} due to no improvement"
            )
...
def decode(
    ...
    test_ids, _, test_queries, test_docs, test_evidence_spans = convert_for_training(
        test, documents, class_interner
    )
        ...
        with torch.no_grad():
            ...
            for batch_start in range(0, len(docs), batch_size):
                ...
                (
                    ...
                    normalized_document_attention,
                ) = classifier(q, None, d, return_attentions=True)
                preds.extend([[y.item() for y in x] for x in classes])
                attentions.extend(
                    [
                        [x.item() for x in y]
                        for y in normalized_document_attention.unpad(
                            normalized_document_attention.data.squeeze()
                        )
                    ]
                )
                unnormalized_attentions.extend(
                    [
                        [x.item() for x in y]
                        for y in unnormalized_document_attention.unpad(
                            unnormalized_document_attention.data.squeeze()
                        )
                    ]
                )
    ...
    def generate_rationales(ids, queries, docs):
        ...
        evidence_only_docs = []
        no_evidence_docs = []
        for (ann_id, docids), doc, pred, attentions in zip(
            ids, docs, *decode_set(queries, docs)
        ):
            doc = np.array(doc)
            classification_scores = {
                class_deinterner[cls]: p for cls, p in enumerate(pred)
            }
            cls = np.argmax(pred)
            if len(docids) == 1:
                (docid,) = docids
        ...
        for i, ((ann_id, _), pred, _) in enumerate(
            zip(ids, *decode_set(queries, evidence_only_docs))
        ):
            classification_scores = {
                class_deinterner[cls]: p for cls, p in enumerate(pred)
            }
        ...
        for i, ((ann_id, _), pred, _) in enumerate(
            zip(ids, *decode_set(queries, no_evidence_docs))
        ):
            classification_scores = {
                class_deinterner[cls]: p for cls, p in enumerate(pred)
            }
